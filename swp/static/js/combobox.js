...
(function($) {
	$.widget("ui.combobox", {
		_create: function() {
			...
			var btn = $("<button><span class='ui-button-icon-primary ui-icon ui-icon-triangle-1-s'></span></button>");
			btn.attr("tabIndex", -1);
			...
			input.css("margin",0);
			input.addClass(select.attr('name'));
			btn.css("padding",0);
			...
			input.css("width","80%");// match the width
		}
	});
...
$(function() {
	$(".combobox").combobox();
	$("div.filterdiv input.ui-autocomplete-input.company").attr('placeholder','Type Company Name');
	$("div.filterdiv input.ui-autocomplete-input.location").attr('placeholder','Type State Name');
});